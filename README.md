...
├── subfinder\_scanner.py       # Découverte de sous-domaines

├── shodan\_scanner.py          # Enrichissement via l'API Shodan

├── httpx\_scanner.py           # Détection technologies/headers HTTP

├── nuclei\_scanner.py          # Scan de vulnérabilités

├── secrets\_scanner.py         # Détection de secrets JS et endpoints sensibles

├── database.py                # Persistance SQLite (schéma + fonctions CRUD)

├── main\_scanner.py            # Orchestrateur du pipeline complet

├── report\_generator.py        # Génération des rapports HTML/PDF
...
SHODAN\_API\_KEY=votre\_cle\_shodan\_ici
...
\- L'enrichissement Shodan dépend du quota de requêtes du compte utilisé (`query\_credits`) — un compte gratuit à 0 crédit renvoie des résultats vides sans bloquer le reste du pipeline.
